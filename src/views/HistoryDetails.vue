<template>
    <div v-if="this.$store.getters.IS_LOGGEDIN" class="max-w-7xl mx-auto">
        <div class="flex flex-col py-5 px-3">
            <base-button @click.native="goBack" class="pl-8 mb-5 flex">&#10094; Go back</base-button>
            <circle-loading :loadingProp="loading" class="mx-auto"/>
                ...
                <div class="flex gap-5 mt-5 w-full relative md:absolute bg-white rounded-md shadow-md">
                    ...
                    <div class="flex flex-col items-center flex-1">
                        ...
                        <div v-for="num in selectedNums" :key="num" class="flex flex-col mb-3">
                            <base-ball :numProp="num" :class="[ drawedNums.includes(num)  ? 'shadow-3d-match sm:shadow-3d-match-sm': 'shadow-3d sm:shadow-3d-sm' ]"/>
                        </div>
                    </div>
                </div>
            </div>
