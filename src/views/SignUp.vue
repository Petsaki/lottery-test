<template>
  <div class="min-h-screen flex justify-center items-center sm:items-start sm:pt-36" v-if="!this.$store.getters.IS_LOGGEDIN">
    <form @submit.prevent="signUpFun" class="flex flex-col h-fit justify-start items-center gap-5 bg-white px-5 sm:px-12  rounded-md border-2 border-black pt-5 pb-8">
      ...
      <base-input labelProp="Confirm password:">
      ...
      <div class="flex justify-center flex-col gap-4 w-fit pt-3">
        <base-button tagProp="input" typeProp="submit" valueProp="Sign up" class="bg-green-400 text-white"/>
        <base-button tagProp="router-link" toProp="/login" class="text-blue-500 underline decoration-blue-500">Log in</base-button>
      </div>
